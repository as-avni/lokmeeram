...
export default function EnhancedWireframeCards() {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4">
      <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">AAyojan</h1>
      <div className="space-y-4">
        {cardData.map((card) => (
          <Card key={card.id} className="w-full">
            <CardHeader>
              <CardTitle className="text-lg font-medium">{card.companyName}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex gap-4">
                ...
                <div
                  ...
                  onClick={() => handleImageClick(card.id, card.imageSrc, card.companyName)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      handleImageClick(card.id, card.imageSrc, card.companyName)
                    }
                  }}
                >
                  <img
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="text-sm">
                    <span className="font-medium">{card.initials}</span>
                  </div>

...
export default function EnhancedWireframeCards() {
  ...
  const handleImageClick = (cardId: number, name: string) => {
    const company = cardData.find((c) => c.id === cardId)
    if (!company) return
    setSelectedImage({
      id: cardId,
  ...
  return (
    ...
    <section className="w-full max-w-2xl mx-auto p-4">
      ...
      <div className="space-y-4">
        {cardData.map((card) => (
          <Card key={card.id} className="w-full p-6 bg-transparent">
            {/* <CardHeader>
            ...
            <CardContent>
              <div className="flex gap-4 items-center">
                ...
                <div
                  ...
                  onClick={() => handleImageClick(card.id, card.name)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      handleImageClick(card.id, card.name)
                    }
                  }}
                >
                  <Image
